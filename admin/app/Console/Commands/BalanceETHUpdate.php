...
class BalanceETHUpdate extends Command
{
    ...
    {           
        ...
        $useraddress = UserEthAddress::where(['eth_status' => 0])->get();
        if(count($useraddress) > 0){
			...
			foreach ($useraddress as $user) {
				...
				UserEthAddress::where(['id' => $user->id])->update(['eth_status' => 1,'updated_at' => date('Y-m-d H:i:s',time())]);
			}
		}else{
            $users = UserEthAddress::where('eth_status',1)->orderBy('user_id','Desc')->get();
            foreach ($users as $user) {
                UserEthAddress::where('user_id',$user->user_id)->update(['eth_status' => 0, 'updated_at' => date('Y-m-d H:i:s',time())]);
            }
        } 
