...
class BalanceMATICTokenUpdate extends Command
{
    ...
    public function handle(){
        $useraddress = UserEthAddress::where(['polytoken_status' => 0])->orderBy('id', 'desc')->get();
        if(count($useraddress) > 0){
            ...
            foreach ($useraddress as $user) {
                ...
                $limit++;
				UserEthAddress::where(['id' => $user->id])->update(['polytoken_status' => 1,'updated_at' => date('Y-m-d H:i:s',time())]);
            }
        }else{
            $users = UserEthAddress::where('polytoken_status',1)->orderBy('user_id','Desc')->get();
            foreach ($users as $user) {
                UserEthAddress::where('user_id',$user->user_id)->update(['polytoken_status' => 0, 'updated_at' => date('Y-m-d H:i:s',time())]);
            }
        }
