...
class BalanceBNBUpdate extends Command
{
    ...
    {           
        ...
        $useraddress = UserEthAddress::where(['bnb_status' => 0])->get();
		$limit = 1;
        if(count($useraddress) > 0){            
			foreach ($useraddress as $user) {
				...
				UserEthAddress::where(['id' => $user->id])->update(['bnb_status' => 1,'updated_at' => date('Y-m-d H:i:s',time())]);
			}
		}else{
            $users = UserEthAddress::where('bnb_status',1)->orderBy('user_id','Desc')->get();
            foreach ($users as $user) {
                UserEthAddress::where('user_id',$user->user_id)->update(['bnb_status' => 0, 'updated_at' => date('Y-m-d H:i:s',time())]);
            }
        }
