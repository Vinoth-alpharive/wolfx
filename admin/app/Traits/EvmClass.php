...
trait EvmClass
{
    ...
    {
        $tokenAPI = "e874d689b69d4d759aa01ce86bd1cabc";
        $ch = curl_init();
    ...
    {
        // $coins = Commission::whereIn('source', ['ETH', 'MATIC', 'BNB'])->get();
        //only eth for Wolf X 
        $coins = Commission::whereIn('source', ['ETH'])->get();
        foreach ($coins as $coin) {
            $symbol = $coin->source;
            $walletaddress = UserWallet::on('mysql2')->where(['uid' => $uid, 'currency' => $symbol])->first();
            $balance = 0;
            if (!$walletaddress) {
    ...
    function walletEVMToken($uid, $address)
    {
        ...
        if (count($coins) > 0) {
            foreach ($coins as $token) {
                $coin = $token->source;
                $walletaddress = UserWallet::on('mysql2')->where(['uid' => $uid, 'currency' => $coin])->first();
                $balance = 0;
                $escrow = 0;
                if (!$walletaddress) {
                    $walletaddress = new UserWallet;
    ...
    function createAdminTransactionEVMToken($network, $contractaddress, $toaddress, $eth_amount)
    {
        ...
        $pvk = Crypt::decryptString($private->narcanru);
        $length = strlen($pvk);
        if ($length == 62) {
            $pvk = '00' . $pvk;
        }
