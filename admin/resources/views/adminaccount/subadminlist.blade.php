  ...
  <div class="card">
    <div class="card-body">
      ...
      <div class="col-md-12 col-sm-12 col-xs-12 pl-0">
        ...
        <a href="{{url('admin/subadminform')}}"  class="btn btn-light"><i class="fa fa-user-plus zmdi-hc-fw"></i> Add Sub Admin </a> </div>
        @endif
      <div class="table-responsive search_result">
        <table class="table downloaddatas" > 
          ...
          <thead>
            <tr>
              ...
              <th>Date & Time</th>
               <!-- <th>logintime</th>
                <th>loginout</th> -->
               @if(in_array("write", explode(',',$subadmin->addadmin)) || in_array("write", explode(',',$subadmin->addadmin)))  
               <th>Action</th>
                @endif
            </tr>
          </thead>
          ...
          <tr>
            <td>{{ $i }}</td>
            <td>{{ $user->name }}</td>
            <td>@php echo $user->email!= "" ?mb_strimwidth($user->email, 0, 100, "...") : ''; @endphp</td>
            ...
            <td class="td-btns">
                ...
                @if(in_array("write", explode(',',$subadmin->addadmin)))  
                <a class="btn btn-success btn-xs" href="{{ url('admin/subadminedit/'.\Crypt::encrypt($user->id)) }}"><i class="zmdi zmdi-edit"></i> Edit </a> &nbsp;  
                @endif
                
                @if(in_array("delete", explode(',',$subadmin->addadmin)))  
                <a style="color: #000" class="btn btn-success btn-xs" data-toggle="modal" data-target="#modal-default2_{{ $user->id }}"><i class="zmdi zmdi-delete"></i> Remove  </a>
                    ...
                    <div class="modal-footer"> <a href="{{ url('admin/subadminremove/'. \Crypt::encrypt($user->id)) }}" class="btn btn-success btn-xs">Delete </a>
