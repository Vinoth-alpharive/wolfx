		    ...
		    <form action="{{ url('/admin/deposit/search') }}" method="get" autocomplete="off">
				...
				<div class="row">
					<div class="col-md-7 d-flex">                
						<input type="text" name="searchitem" class="form-control" placeholder="Search for User Name or Email" value= "{{ $q ? $q : '' }}" />
					...
					{{-- <div class="col-md-2">
						<a class="btn btn-warning" href="{{ route('depositexport',['coin' => $coin ]) }}">Export To Excel</a>
					</div> --}}
				</div>
				...
				<table class="table" id="dows">
					...
					@foreach($depositList as $key => $histroy)
						<tr>
							...
							<td><a href="{{ url('admin/users_edit/'.Crypt::encrypt($histroy->uid ?? '')) }} ">{{ $histroy->user['username'] ?? ''}}</a></td>
