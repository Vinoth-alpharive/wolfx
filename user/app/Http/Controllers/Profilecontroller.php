...
class Profilecontroller extends Controller
{
    ...
    {
        $user = Auth::id();
        $walletbalance = Wallet::where(['uid' => $user, 'currency' => 'USDT'])->first();
        $total_rewards = AffliateTransaction::where('uid', $user)->sum('price');
        return view('layouts.profile', (['wallet' => $walletbalance, 'total_rewards' => $total_rewards]));
    }
    ...
    {
        $user = Auth::id();
        $purchase = SellRequest::where(['uid' => $user])->orderBy('created_at', 'DESC')->get();
        return view('exchange.purchasehistory', (['purchaseHistory' => $purchase]));
    }

    public function purchaseHistoryUpdate()
    {
        $user = Auth::id();
        $purchase = SellRequest::where(['uid' => $user])->orderBy('created_at', 'DESC')->get();
